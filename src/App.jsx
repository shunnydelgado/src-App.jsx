...
export default function CuraManage() {
  ...
  async function sendAI(text) {
    ...
    setAiLoading(true);
    try {
      const res=await fetch("/.netlify/functions/claude",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-3-5-haiku-20241022",max_tokens:1000,system:SYSTEM_PROMPT,messages:[...aiMsgs,msg].slice(-16)})});
  ...
  async function processFile(file) {
    ...
    reader.onload=async(ev)=>{
      ...
      try{const res=await fetch("/.netlify/functions/claude",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-3-5-haiku-20241022",max_tokens:1000,messages:[{role:"user",content}]})});const data=await res.json();setDocResult({text:data.content?.[0]?.text||"No se pudo extraer.",filename:file.name});}
